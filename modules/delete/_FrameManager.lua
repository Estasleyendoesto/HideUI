...
function FrameManager:GlobalSettingsUpdate(msg, field, input)
    for _, frame  in pairs(GAME_FRAMES) do
        if frame and frame.HideUI then
            ...
            elseif self:IsEventSetting(field) then
                frame.HideUI:OnEventConfig(field, "Global")
            end
        end
    end
end
...
function FrameManager:FrameSettingsUpdate(msg, frame_name, field, input)
    ...
    if frame and frame.HideUI then
        ...
        elseif field == "isAlphaEnabled" then
            frame.HideUI:OnAlphaConfig("Custom")
        elseif self:IsEventSetting(field) then
            frame.HideUI:OnEventConfig(field, "Custom")
...
function FrameManager:IsEventSetting(field)
